...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            pic_profile = save_picture(form.picture.data)
            current_user.image_file = pic_profile

        current_user.username = form.username.data
        current_user.email = form.email.data

        db.session.commit()
        flash(f"You've Successfully Updates", 'success')
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash("An Email Has Been Sent With The INstruction to Reset Your Password",'info')
        return redirect(url_for('login'))
